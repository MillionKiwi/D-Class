    ...
    <div class="review-write">
      ...
      <section class="review-write__section">
        <div class="academy-summary">
          <div class="academy-summary__icon">
...
onMounted(async () => {
  try {
    // 학원 정보 가져오기
    const response = await apiClient.get(`/academies/${route.params.academyId}/basic`)
    academyName.value = response.data.name
    workPeriod.value = response.data.workPeriod || ''
  } catch (err) {
    console.error('Failed to load academy info:', err)
  }
})
</script>
